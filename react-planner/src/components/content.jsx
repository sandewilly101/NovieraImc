...
export default function Content({width, height, state, customContents}) {
  ...
  const custom = customContents || {};

  switch (mode) {
    case constants.MODE_3D_VIEW:
      if (custom.hasOwnProperty(constants.MODE_3D_VIEW)) {
        let Custom3D = custom[constants.MODE_3D_VIEW];
        return <Custom3D width={width} height={height} state={state} />;
      }
    ...
    case constants.MODE_3D_FIRST_PERSON:
      if (custom.hasOwnProperty(constants.MODE_3D_FIRST_PERSON)) {
        let CustomFP = custom[constants.MODE_3D_FIRST_PERSON];
        return <CustomFP width={width} height={height} state={state} />;
      }
