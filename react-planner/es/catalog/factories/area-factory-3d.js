import { Shape, MeshPhongMaterial, ShapeGeometry, Box3, TextureLoader, Mesh, MeshBasicMaterial, RepeatWrapping, Vector2, DoubleSide, BufferAttribute } from 'three';
...
var assignUVs = function assignUVs(geometry) {
  ...
  var rx = max.x - min.x;
  var ry = max.y - min.y;
  var range = new Vector2(rx || 1, ry || 1);
  ...
  for (var i = 0; i < pos.count; i++) {
    var x = pos.getX(i);
    var y = pos.getY(i);
    uv[i * 2] = (x + offset.x) / range.x;
    uv[i * 2 + 1] = (y + offset.y) / range.y;
  }
