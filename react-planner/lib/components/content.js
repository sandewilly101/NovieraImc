...
function Content(_ref) {
  ...
  var custom = customContents || {};

  switch (mode) {
    case constants.MODE_3D_VIEW:
      if (custom.hasOwnProperty(constants.MODE_3D_VIEW)) {
        var Custom3D = custom[constants.MODE_3D_VIEW];
        return _react2.default.createElement(Custom3D, { width: width, height: height, state: state });
      }
    ...
    case constants.MODE_3D_FIRST_PERSON:
      if (custom.hasOwnProperty(constants.MODE_3D_FIRST_PERSON)) {
        var CustomFP = custom[constants.MODE_3D_FIRST_PERSON];
        return _react2.default.createElement(CustomFP, { width: width, height: height, state: state });
      }
    ...
    default:
      if (custom.hasOwnProperty(mode)) {
        var CustomContent = custom[mode];
