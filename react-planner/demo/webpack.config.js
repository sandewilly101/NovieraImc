...
module.exports = (env, self) => {
  ...
  /** Prefer PLANNER_DEV_PORT when 9000 is busy (e.g. `cross-env PLANNER_DEV_PORT=9001 npm start`). */
  const fromEnv = parseInt(process.env.PLANNER_DEV_PORT || '', 10);
  let port =
    Number.isFinite(fromEnv) && fromEnv > 0
      ? fromEnv
      : self.hasOwnProperty('port') && self.port != null
        ? Number(self.port)
        : 9000;
  ...
  let config = {
    ...
    output: {
      ...
      // Set PLANNER_PUBLIC_PATH=/react-planner/ when copying demo/dist into Novira's public/react-planner/
      publicPath: process.env.PLANNER_PUBLIC_PATH || '/',
    },
    performance: {
